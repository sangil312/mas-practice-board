...
[클래스 책임 및 역할]
- Business Layer는 여러 Component를 조합하는 책임만 가진다.
- Implement Layer에서 하나의 class는 하나의 책임만 가져야한다.
...
- 쓰기 함수더라도 @Transactional 이 필요 없다면 사용하지 않는다.
    - 단일 JPA Entity 의 *Repository.save() 등의 원자성이 필요하지 않은 로직
...
- 로직이 필요한 변환의 경우 `*ApiResponse.of(...)` 함수를 사용한다.
    - 복수 파라미터가 들어갈 수 있다.
